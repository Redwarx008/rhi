...
namespace rhi::impl
{
    ...
    inline typename std::conditional<sizeof(T1) >= sizeof(T2), T1, T2>::type AlignUp(T1 val, T2 alignment)
    {
        ...
        return (static_cast<ResultType>(val) + static_cast<ResultType>(alignment - 1)) & ~static_cast<ResultType>(
            alignment - 1);
    }


    template <typename T>
    [[nodiscard]] bool arraysAreDifferent(const T* a, uint32_t a_len, const T* b, uint32_t b_len)
    {
        if (a_len != b_len)
        {
            return true;
