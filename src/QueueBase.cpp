...
namespace rhi::impl
{
    ...
    uint64_t QueueBase::APISubmit(CommandListBase* const* commands,
    ...
    uint64_t ComputeRequiredBytesInCopy(TextureFormat format,
                                        const Extent3D& size,
    ...
    void QueueBase::APIWriteTexture(const TextureSlice& dstTexture,
                                    const void* data,
                                    size_t dataSize,
                                    const TextureDataLayout& dataLayout)
    {
        ...
        INVALID_IF(dataLayout.offset > dataSize,
                   "Data offset (%u) is greater than the data size (%u).",
                   dataLayout.offset,
                   dataSize);
        ...
        CopyTextureData(copyDst,
                        copySrc,
                        dstTexture.size.depthOrArrayLayers,
                        alignedRowsPerImage,
                        additionalStridePerImage,
                        alignedBytesPerRow,
                        optimallyAlignedBytesPerRow,
